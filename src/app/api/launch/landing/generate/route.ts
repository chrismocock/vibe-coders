...
import { parseLandingResponse } from "@/lib/launch/parsers";
...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt =
      (config as any)?.system_prompt_launch_landing ||
      (fallbackConfig as any)?.system_prompt_launch_landing ||
      config?.system_prompt ||
      fallbackConfig?.system_prompt ||
      "";

    let userPrompt = `Generate landing page and onboarding content.\n\n`;
    ...
    const parsedResult = parseLandingResponse(result);
      ...
      .upsert(
        {
          ...
          user_id: userId,
          landing_onboarding: parsedResult,
