...
import { parseAdoptersResponse } from "@/lib/launch/parsers";
...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt =
      (config as any)?.system_prompt_launch_adopters ||
      (fallbackConfig as any)?.system_prompt_launch_adopters ||
      config?.system_prompt ||
      fallbackConfig?.system_prompt ||
      "";

    let userPrompt = `Generate early adopter profiles and outreach plan.\n\n`;
    ...
    const parsedResult = parseAdoptersResponse(result);
    ...
    return NextResponse.json(
      { error: "Failed to generate early adopter plan" },
      { status: 500 }
    );
  }
}
