...
import { parseOfferResponse } from "@/lib/monetise/parsers";
...
export async function POST(request: Request) {
  try {
    ...
    const { projectId, planType } = body; // planType: "simple" | "multi_tier"
    ...
    const systemPrompt =
      (config as any)?.system_prompt_monetise_offer ||
      (fallbackConfig as any)?.system_prompt_monetise_offer ||
      config?.system_prompt ||
      fallbackConfig?.system_prompt ||
      "";

    // Build user prompt
    let userPrompt = `Generate offer and plan structure.\n\n`;
    userPrompt += contextPrompt;
    
    if (planType) {
      userPrompt += `\nPlan Type: ${planType === "simple" ? "Single offer" : "Multi-tier plans"}\n`;
    }
    ...
    const parsedResult = parseOfferResponse(result);

    // Update blueprint
    const sectionCompletion = monetiseBlueprint?.section_completion || {};
    const updateData: any = {
      offer_plan: parsedResult,
    ...
    return NextResponse.json(
      { error: "Failed to generate offer plan" },
      { status: 500 }
    );
  }
}
